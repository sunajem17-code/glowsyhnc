...
Any field that affects score, premium status, subscription, leaderboard rank, or access to paid content must be **computed server-side and never accepted from the client request body**. If a route receives a score or `isPremium` value from `req.body`, that is a bug. Derive it from Codex's response, the database, or RevenueCat's API — never from what the client tells you.
...
All Codex API routes (`/api/ai/*`, `/api/coach/*`) must use `createLimiter` backed by Upstash Redis. In-memory fallback is only acceptable in local dev. Production startup throws if Redis env vars are missing.
